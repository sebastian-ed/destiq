...
let individualChartMode = 'yearly';
let individualZoomState = { signature: '', start: 0, span: null };
...
async function selectIndicator(id) {
  ...
  try {
    ...
    individualChartMode = 'yearly';
    individualZoomState = { signature: '', start: 0, span: null };
    renderDashboard();
    document.getElementById('mainEmpty').style.display = 'none';
...
function renderDashboard() {
  ...
  const historyMode = individualChartMode === 'history';
  updateIndividualChartHeader(historyMode);
  ...
  if (hasVisibleYears) {
    if (historyMode) {
      renderIndividualHistoricalChart(shownYears);
    } else {
      renderIndividualZoomControls([], false);
...
function updateIndividualChartHeader(historyMode = individualChartMode === 'history') {
  const titleEl = document.getElementById('lineChartTitle');
  const toggleBtn = document.getElementById('individualChartModeToggle');
  if (titleEl) titleEl.textContent = historyMode ? 'Evolución mensual histórica' : 'Evolución mensual por año';
  if (toggleBtn) toggleBtn.textContent = historyMode ? 'Ver por año' : 'Ver histórica';
}

function toggleIndividualChartMode() {
  individualChartMode = individualChartMode === 'history' ? 'yearly' : 'history';
  individualZoomState = { signature: '', start: 0, span: null };
  renderDashboard();
}

function getIndividualMonthlyPeriods(yearsFilter = getVisibleYears()) {
  const periodKeys = [];
  const allowedYears = new Set((yearsFilter || []).map(year => String(year)));
  Object.keys(currentDataByYear || {}).sort((a, b) => Number(a) - Number(b)).forEach(year => {
    if (allowedYears.size && !allowedYears.has(String(year))) return;
    (currentDataByYear[year] || []).forEach((value, monthIndex) => {
      if (value === null || value === undefined || isNaN(Number(value))) return;
      periodKeys.push(`${year}-${String(monthIndex + 1).padStart(2, '0')}`);
    });
  });
  return periodKeys.sort((a, b) => a.localeCompare(b));
}

function getIndividualMonthlyValue(periodKey) {
  const [year, month] = String(periodKey || '').split('-');
  const value = currentDataByYear?.[Number(year)]?.[Number(month) - 1];
  return value === null || value === undefined || isNaN(Number(value)) ? null : Number(value);
}

function getIndividualZoomSignature(labels = []) {
  return `${currentIndicator?.id || ''}|${labels.length}|${labels[0] || ''}|${labels[labels.length - 1] || ''}|${getVisibleYears().join(',')}`;
}

function resetIndividualZoom(labels = []) {
  individualZoomState = {
    signature: getIndividualZoomSignature(labels),
    start: 0,
    span: labels.length || null,
  };
}

function clampIndividualZoom(total) {
  if (!total) {
    individualZoomState.start = 0;
    individualZoomState.span = null;
    return;
  }
  const minSpan = Math.min(6, total);
  let span = Number(individualZoomState.span);
  if (!Number.isFinite(span) || span < minSpan) span = total;
  span = Math.max(minSpan, Math.min(total, Math.round(span)));
  let start = Number(individualZoomState.start);
  if (!Number.isFinite(start)) start = 0;
  start = Math.max(0, Math.min(total - span, Math.round(start)));
  individualZoomState.span = span;
  individualZoomState.start = start;
}

function getIndividualZoomWindow(labels = []) {
  const total = labels.length;
  const signature = getIndividualZoomSignature(labels);
  if (individualZoomState.signature !== signature) {
    resetIndividualZoom(labels);
  }
  clampIndividualZoom(total);
  const start = individualZoomState.start || 0;
  const span = individualZoomState.span || total;
  return {
    start,
...
function applyIndividualZoomToSeries(labels, series) {
  const windowInfo = getIndividualZoomWindow(labels);
  if (!windowInfo.isZoomed) return { labels, series, windowInfo };
...
function renderIndividualZoomControls(labels = [], historyMode = individualChartMode === 'history') {
  const controls = document.getElementById('individualZoomControls');
  if (!controls) return;
  if (!historyMode || labels.length <= 18) {
  ...
  controls.innerHTML = `
    <div class="comparison-zoom-main">
      ...
      <button class="btn btn-secondary btn-sm" type="button" onclick="moveIndividualZoom(-1)" ${windowInfo.start <= 0 ? 'disabled' : ''}>←</button>
      <button class="btn btn-secondary btn-sm" type="button" onclick="changeIndividualZoom(1)">+ Zoom</button>
      <button class="btn btn-secondary btn-sm" type="button" onclick="changeIndividualZoom(-1)" ${!windowInfo.isZoomed ? 'disabled' : ''}>− Zoom</button>
      <button class="btn btn-ghost btn-sm" type="button" onclick="resetIndividualZoomAndRender()" ${!windowInfo.isZoomed && windowInfo.start === 0 ? 'disabled' : ''}>Ver todo</button>
      <button class="btn btn-secondary btn-sm" type="button" onclick="moveIndividualZoom(1)" ${windowInfo.end >= windowInfo.total ? 'disabled' : ''}>→</button>
    </div>
    <div class="comparison-zoom-range-wrap">
      <input class="comparison-zoom-range" type="range" min="0" max="${maxStart}" value="${windowInfo.start}" ${maxStart <= 0 ? 'disabled' : ''} oninput="setIndividualZoomStart(this.value)"/>
...
function getIndividualHistoricalLabelsAndSeries(years = getVisibleYears()) {
  const periods = getIndividualMonthlyPeriods(years);
  const labels = periods.map(formatComparisonPeriodLabel);
  const series = [{
    label: currentDestination ? getDestinationDisplayName(currentDestination) : (currentIndicator?.name || 'Serie histórica'),
    values: periods.map(getIndividualMonthlyValue),
  }];
  return { labels, series };
}

function renderIndividualHistoricalChart(years = getVisibleYears()) {
  const raw = getIndividualHistoricalLabelsAndSeries(years);
  const zoomed = applyIndividualZoomToSeries(raw.labels, raw.series);
  renderIndividualZoomControls(raw.labels, true);
  renderHistoricalLineChart('lineChart', zoomed.labels, zoomed.series, currentIndicator, 'Evolución mensual histórica');
}

function changeIndividualZoom(direction) {
  if (individualChartMode !== 'history') return;
  const labels = getIndividualHistoricalLabelsAndSeries(getVisibleYears()).labels;
  const windowInfo = getIndividualZoomWindow(labels);
  ...
  individualZoomState.span = nextSpan;
  individualZoomState.start = Math.max(0, Math.min(windowInfo.total - nextSpan, Math.round(center - nextSpan / 2)));
  renderDashboard();
}

function moveIndividualZoom(direction) {
  if (individualChartMode !== 'history') return;
  const labels = getIndividualHistoricalLabelsAndSeries(getVisibleYears()).labels;
  const windowInfo = getIndividualZoomWindow(labels);
  const step = Math.max(1, Math.round(windowInfo.span * 0.5));
  individualZoomState.start = Math.max(0, Math.min(windowInfo.total - windowInfo.span, windowInfo.start + (direction * step)));
  renderDashboard();
}

function setIndividualZoomStart(value) {
  if (individualChartMode !== 'history') return;
  const labels = getIndividualHistoricalLabelsAndSeries(getVisibleYears()).labels;
  const windowInfo = getIndividualZoomWindow(labels);
  individualZoomState.start = Math.max(0, Math.min(windowInfo.total - windowInfo.span, Number(value) || 0));
  renderDashboard();
}

function resetIndividualZoomAndRender() {
  const labels = individualChartMode === 'history' ? getIndividualHistoricalLabelsAndSeries(getVisibleYears()).labels : [];
  resetIndividualZoom(labels);
  renderDashboard();
}
