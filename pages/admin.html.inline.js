...
function getComparisonMonthlyPeriods(payload = currentComparisonTablePayload) {
...
function buildComparisonChartSeries(payload) {
  ...
  if (isMonthlyComparisonMode()) {
    const periods = getComparisonMonthlyPeriods(payload);
...
function renderComparisonChartFromPayload(payload = currentComparisonTablePayload) {
  ...
  const periods = monthlyMode ? getComparisonMonthlyPeriods(payload) : [];
  renderComparisonChart('comparisonChart', {
    years: monthlyMode ? [] : (payload.years || []),
...
function renderMonthlyComparisonTable(payload, table) {
  ...
  if (!periods.length) {
    table.innerHTML = '<tbody><tr><td>No hay datos mensuales cargados para esta comparación.</td></tr></tbody>';
    return;
  }
